...
export async function getExamesPorCPF(cpf, pesquisa) {
    ...
    try {
        var response;
        if (pesquisa) {
            response = await fetch(`${url}/Exams?cpf=${cpf}&pesquisa=${pesquisa}`, request);
        } else {
            response = await fetch(`${url}/Exams?cpf=${cpf}`, request);
        }
